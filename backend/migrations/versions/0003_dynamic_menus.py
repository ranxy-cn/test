...
def upgrade() -> None:
    ...
    # 幂等：库表可能已按 backend/migrations/ddl/0003_dynamic_menus.sql 手工建过，
    # 仅在缺失时创建，避免 "Table 'menus' already exists" 卡死启动。
    ...
    if "role_menus" not in existing_tables:
        op.create_table(
            "role_menus",
            sa.Column(
                "role_id",
    ...
    from app.models import Permission, Role, RoleMenu
    ...
    try:
        ...
        for role_code in ("admin", "operator", "viewer"):
            ...
            # 守卫：已有自定义授权的角色不重置（与 seed.py 逻辑一致）
            if role is not None and db.scalar(
                sa.select(sa.func.count()).where(RoleMenu.role_id == role.id)
            ) == 0:
