<template>
  <div v-if="report">
    <div class="card-grid">
      ...
      <el-card shadow="never"><b>备份</b><div>{{ report.backups.status }}（{{ report.backups.backup_ok_count || 0 }} 成功 / {{ report.backups.restore_verified_count || 0 }} 已验证）</div></el-card>
    </div>
    ...
    <el-card header="未结事项（含升级交接）" style="margin-top: 16px">
      ...
      <el-table :data="report.backups.items || []" size="small" style="margin-top:8px">
        <el-table-column prop="name" label="备份任务" />
        <el-table-column prop="backup_status" label="备份" width="90" />
